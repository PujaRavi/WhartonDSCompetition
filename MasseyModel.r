...
library(dplyr)
# Load the games data
games_data <- read.csv("data/games_2022 v2_80.csv", header = TRUE) # nolint
...
for (i in 1:nrow(games_data_unique)) { # nolint
  ...
  new_df <- data.frame(
    ...
    home = game$team,
    away = game$opponent_team_score,
    margin = margin
  )
  massey_data <- rbind(massey_data, new_df)
}
...
if (det(M) == 0) {
  ...
  rankings <- solve(M, p)
  write.csv(rankings, "data/MASSEY RANKINGS.csv")
}
write.csv(massey_data, "data/MASSEY DATA.csv")
